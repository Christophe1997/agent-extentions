...
### Step 3: Check Archetype and Create Post

**Step 3a: Find and read the archetype**

Check for existing archetypes using Hugo's lookup order:

```bash
# Check for posts-specific archetype first
ls -la archetypes/posts.md themes/*/archetypes/posts.md 2>/dev/null

# If not found, check for default archetype
ls -la archetypes/default.md themes/*/archetypes/default.md 2>/dev/null
```

If an archetype exists, read it to understand what front matter fields it provides.

**Step 3b: Create the post using Hugo**
...
This automatically generates a file with:
- Correct date in the user's local timezone
- Front matter structure from the archetype (if exists)

**Step 3c: Add missing front matter fields**

After Hugo creates the file, read it and supplement only the fields that are missing.
...
**Example - Archetype provides title, date, categories:**
```yaml
---
title: "文章标题"
date: <auto-generated by hugo new>
categories: ["Things I Learned"]
# Add only these missing fields:
draft: true
author: "<AgentName>"
showToc: true
...
**Important:** Respect the user's archetype structure. Only add fields that are missing, don't modify existing archetype fields.
