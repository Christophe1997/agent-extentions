...
- **`matches`** is a *list of inline tables*. The rule fires if **any one entry** matches.
...
- Use `{ }` (empty inline table) to mean "match any input" (e.g., when the `tool` regex is enough).
...
1. If `./.yapermission.toml` exists in the cwd, it is the **only** active config (replaces global).
2. Otherwise, `~/.yapermission.toml` is used.
3. Otherwise, every call falls through to Claude Code's normal permission prompt.
4. If the active TOML fails to parse, the hook logs the error and falls back to `ask` (fail-open).
